...
const UnpaidTotal: React.FC<UnpaidTotalProps> = ({
  invoices,
  filters
}) => {
  const unpaidTotal = invoices.filter(inv => {
  ...
  return <Card className="bg-amber-50">
      <CardHeader>
        <CardTitle className="text-amber-800">Total of unpaid invoices</CardTitle>
      </CardHeader>
      <CardContent>
